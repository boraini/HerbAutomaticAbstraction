#
# Modifications have been made to accept a custom fitness function.
#
...
function get_best_program(population::Array{RuleNode}, iter::GeneticSearchIterator)::RuleNode
    ...
    for index ∈ eachindex(population)
        ...
        # Calculate fitness'
        zipped_outputs = begin
            outputs = [(try execute_on_input(get_grammar(iter.solver), chromosome, example.in) catch e
                ""
            end) for example in iter.spec]
            zip([example.out for example in iter.spec], outputs)
        end

        fitness_value = iter.fitness_function(iter, chromosome, collect(zipped_outputs))
...
function Base.iterate(iter::GeneticSearchIterator, current_state::GeneticIteratorState)
    ...
    # Calculate fitness'
    zipped_outputs(chromosome) = begin
        outputs = [(try execute_on_input(get_grammar(iter.solver), chromosome, example.in) catch e begin
            ""
        end end) for example in iter.spec]
        return zip([example.out for example in iter.spec], outputs)
    end

    fitness_array = [iter.fitness_function(iter, chromosome, collect(zipped_outputs(chromosome))) for chromosome in current_population]
